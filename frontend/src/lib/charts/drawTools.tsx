/**
 * Drawing-tool catalogue for the /charts workspace rail.
 *
 * A group is one rail button that opens a flyout of its tools. Tool ids are the
 ...
 *
 * Deliberately a copy of `lib/trading/drawTools.tsx` rather than an import.
 * `/trading` is reference-only, and keeping a separate catalogue lets this
 * workspace regroup or relabel without any risk to it. The cost is that a tool
 * added to the library has to be added in both places — so
 * `drawTools.test.ts` compares this catalogue against
 * `registeredDrawingTools()` and fails when they drift. That drift is not
 * hypothetical: the rail this replaced offered 18 of 43 tools because a
 * hand-written list silently fell behind the library.
 */
