/**
 * Pure geometry and formatting behind the GEX Levels chart primitives
 * (`gex-levels-primitive.ts`): price/money formatting, where an off-screen
 * level's stub lands, the per-strike bar column's layout and hit-testing,
 * and the hover readout's text content and box placement.
 *
 * Nothing here imports `openalgo-charts` or touches
 * `CanvasRenderingContext2D` - every function takes plain numbers and
 * callbacks (`priceToY`, not the library's `PriceScale`) so it can be
 * unit-tested without a canvas or a chart. `gex-levels-primitive.ts` is the
 * only consumer that turns this into pixels on screen; `GexDashboard.tsx`
 * (the sidebar card) imports `formatGexMoney` from here directly rather than
 * through the canvas module - the inverted dependency ("a components file
 * imports the canvas paint module for a currency formatter") that existed
 * before this file did.
 */

import type { GEXStrikeLevel, GexMetric } from '@/api/gex'
 ...
 * compared at a glance (see the module doc comment on `gex-levels-primitive.ts`
 * - strike 24000 being -1318 Cr gamma but +679 Cr delta is the entire point
 * of the feature), and an unsigned positive number there would read as
 * ambiguous rather than as "the opposite of the row above it", which is
 * usually the case.
 */
 ...
 *
 * `GexOverlayPrimitive` draws no caption when the bar column is switched off
 * (`showBars`) or when the manager's most recent snapshot has nothing to
 * show (`hasBars` - no data yet, an error response, or an instrument with no
 * option chain: see `GexLevelsManager.captionOptions()`), and that absence
 * is fine - there is then nothing on screen for it to disambiguate. It does
 * NOT cover a viewport panned or zoomed away from every strike in an
 * otherwise-good snapshot: telling visible strikes from off-screen ones
 * needs the price scale, which is genuine draw-time knowledge the
 * options-level `hasBars` gate does not have - and that case is the benign
 * one anyway, since the walls stay on screen and the study is plainly still
 * live.
 */
