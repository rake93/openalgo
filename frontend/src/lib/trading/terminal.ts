...
export interface TerminalCallbacks {
  ...
  /**
   * The **library tier's** indicator list changed.
   *
   * Two indicator runtimes coexist on this terminal and neither subsumes the
   * other: the chart library's own `/indicators` tier, reported here, and the
   * OpenScript WASM engine, reported by {@link onEngineIndicators}. They are
   * added, listed, styled and removed through different APIs, so a single
   * callback would have to lie about one of them.
   */
  onIndicatorsChange?(list: { id: string; name: string }[]): void
  /**
   * The **engine-backed** indicator list changed (add/remove/inputs/error).
   * Carries the whole instance because these report an `error` the host shows.
   */
  onEngineIndicators?(list: IndicatorInstance[]): void
  /**
...
export class TradingTerminal {
  ...
  constructor(opts: TerminalOptions) {
    ...
    this.indicators = new IndicatorHost({
      onIndicators: (list) => {
        ...
        this.cb.onEngineIndicators?.(list)
      },
  ...
  /**
   * Engine-backed indicators. Distinct from {@link listIndicators}, which lists
   * the chart library's own tier — the two runtimes are separate registries.
   */
  listEngineIndicators(): IndicatorInstance[] {
