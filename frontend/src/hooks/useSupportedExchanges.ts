...
export function useSupportedExchanges() {
  ...
  return useMemo(() => {
    ...
    // pricing-underlying resolver, which is expiry-aware and picks the future
    // live at the option's own expiry, so chains, quotes and expiries all work.
    // CDS, BCD and NCDEX stay out until those routes implement their
    // venue-specific flows. CRYPTO is retained for crypto-only brokers.
