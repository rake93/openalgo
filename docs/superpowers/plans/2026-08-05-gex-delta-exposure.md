...
>
> - Under gamma the longest bar always lands on a wall *by construction*: `find_walls` takes `max(net_gex)` and `min(net_gex)`, and the geometry scales `max(abs(exposure))` to exactly `columnWidth`, so the full-length bar is whichever wall carries the larger magnitude. (Corrected during Task 8 — it is not always the Call Wall; when the put side is more extreme, which is routine, the longest bar is the Put Wall.) Under delta that coincidence breaks entirely: the full-length bar is generally a strike with no line on it at all, in the same colour as the wall lines. A user reads that as a second wall the dashboard forgot, or as a broken study.
